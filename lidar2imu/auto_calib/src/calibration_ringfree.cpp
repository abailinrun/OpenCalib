 ...
 *
 * Ring-free calibration implementation for MEMS LiDAR support
...
void RingFreeCalibrator::LoadTimeAndPoes(const std::string &filename,
...
Eigen::Matrix4d RingFreeCalibrator::GetDeltaTrans(double R[3], double t[3]) {
...
void RingFreeCalibrator::Calibration(const std::string lidar_path,
  ...
  LOGI("Using Ring-Free PCA-based feature extraction (MEMS LiDAR compatible)");
  LOGI("Loaded %lu frames", lidar_files_.size());
  ...
  for (int i = 0; i < frm_start_box.size(); i++) {
    ...
    for (size_t frmIdx = 0; frmIdx < frmnum; frmIdx++) {
      ...
      // Use ring-free PCA feature extraction instead of LOAM-style
      if (!genPcdFeature_RingFree(cloud, pl_surf, pl_surf_sharp, pl_corn)) {
        std::cerr << "Feature extraction failed for: " << lidar_file_name << std::endl;
        continue;
      }
  ...
  std::cout << "Ring-Free PCA calibration cost "
  ...
  fCalib << "refined calib (ring-free PCA method):" << std::endl;
...
void RingFreeCalibrator::SaveStitching(const Eigen::Matrix4d transform,
